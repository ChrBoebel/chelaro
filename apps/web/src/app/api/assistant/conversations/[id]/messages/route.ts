...
export async function GET(request: Request, context: Context): Promise<Response> {
  ...
  const before = requestUrl.searchParams.get("before_sequence");
  if (before && /^\d{1,10}$/.test(before)) upstream.set("before_sequence", before);
  const suffix = upstream.size > 0 ? `?${upstream}` : "";
  return proxyFinanceJson(
    `/api/v1/assistant/conversations/${encodeURIComponent(id)}/messages${suffix}`,
  );
}
