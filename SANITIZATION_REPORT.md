...
- vollständige erreichbare Feature-Branch-Historie mit drei Commits und genau einem Root-Commit;
  der daraus erzeugte öffentliche Squash-Stand hat zwei erreichbare Commits und weiterhin genau
  einen Root-Commit;
...
- Gitleaks 8.30.1 scannte vor dem Merge alle drei erreichbaren Branch-Commits und nach dem Squash
  erneut beide erreichbaren öffentlichen Commits mit rund 5,2 MB Git-Inhalt.
...
- Absolute private Benutzerpfade für macOS, Linux und Windows: **0 Treffer** im Current Tree, in
  allen drei geprüften Branch-Commits und in beiden erreichbaren öffentlichen Commits.
...
- Erreichbare Historie auf dem veröffentlichten `main`: genau zwei Commits und genau ein
  Root-Commit. Der aus Pull Request #1 erzeugte Squash-Commit hat ausschließlich den öffentlichen
  Root-Commit als Parent und enthält den geprüften Asset- sowie Reportstand.
- Author und Committer beider öffentlichen Commits:
  `Christopher Böbel <ChrBoebel@users.noreply.github.com>`.
- Der geprüfte Branch `feat/restore-public-brand-assets` hatte vor dem Squash drei erreichbare
  Commits und einen sauberen Arbeitsbaum; der öffentliche Stand hat **0 Tags**.
- `origin` zeigt auf das bereits öffentliche Repository `ChrBoebel/chelaro`; die Remote wurde
  nicht verändert.
- Nach der Bereinigung und erneut auf dem korrigierten Squash-Stand meldete
  `git fsck --full --no-reflogs --unreachable` **0 fehlende, beschädigte oder unerreichbare
  Objekte**. Der öffentliche Root-Commit wurde nicht umgeschrieben.
