...
| Local chat history leaks or grows without bound | Owner-scoped local database; full visible messages only; paginated reads; no raw chunks/reasoning/tool payloads in history or audit; explicit archive/delete; Codex thread deletion precedes Chelaro purge | Local database and shared Codex home are not encrypted by Chelaro; OS-account compromise remains out of scope |
| A restarted chat binds to the wrong provider context | Conversation-to-thread binding is unique; exact `thread/resume`; returned ID and restrictive response projection are validated; no silent new-thread fallback | Manual external deletion of the Codex thread causes explicit context loss |
